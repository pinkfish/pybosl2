...
def test_pitch_and_module() -> None:
    assert pitch_value(2) == pytest.approx(2 * math.pi)
    assert module_value(math.pi) == pytest.approx(1.0)
    assert circular_pitch(mod=2) == pytest.approx(2 * math.pi)
    assert diametral_pitch(mod=2) == pytest.approx(math.pi / (2 * math.pi))


def test_pitch_radius() -> None:
    assert pitch_radius(pitch=5, teeth=11) == pytest.approx(5 * 11 / math.pi / 2)
    # metric: mod*teeth/2
    assert pitch_radius(mod=2, teeth=20) == pytest.approx(20.0)


def test_outer_radius() -> None:
    # outer = pitch_radius + adendum(=module); mod 2, teeth 16 -> 16 + 2 = 18 (no profile shift)
    assert outer_radius(mod=2, teeth=16, profile_shift=0) == pytest.approx(18.0)
    # with the default "auto" profile shift, a 16-tooth gear grows by x*module to avoid undercut
    assert outer_radius(mod=2, teeth=16) > 18.0


def test_bevel_pitch_angle() -> None:
    # atan(sin(90)/((mate/teeth)+cos(90))) = atan(teeth/mate)
    assert bevel_pitch_angle(18, 30) == pytest.approx(math.degrees(math.atan(18 / 30)))


def test_worm_gear_thickness_positive() -> None:
    assert worm_gear_thickness(pitch=5, teeth=36, worm_diam=30) > 0
...
def test_low_tooth_gear_has_undercut_shift() -> None:
    ...
    assert auto_profile_shift(8) > 0.4
...
def test_ring_gear_builds_as_annulus() -> None:
    ...
    expect = 2 * (outer_radius(circ_pitch=5, teeth=20, internal=True) + 3)
...
def test_bevel_gear_envelope() -> None:
    ...
    width, _, height = _size(solid)
    assert width == pytest.approx(2 * pitch_radius(5, 20), abs=3)
    assert height > 1
...
def test_worm_gear_thickness_matches_helper() -> None:
    ...
    thick = _size(solid)[2]
    assert thick == pytest.approx(worm_gear_thickness(5, 30, 25), abs=0.5)
...
def test_herringbone_envelope_matches_spur() -> None:
    ...
    assert width == pytest.approx(2 * outer_radius(mod=5, teeth=20), abs=1.5)
...
def test_auto_profile_shift_formula() -> None:
    ...
    assert auto_profile_shift(17, 20) == pytest.approx(0, abs=0.02)
    assert auto_profile_shift(10, 20) == pytest.approx(1 - 10 * math.sin(math.radians(20)) ** 2 / 2)
    assert auto_profile_shift(30, 20) == 0.0
...
def test_pitch_inputs_agree() -> None:
    ...
    a = pitch_radius(mod=5, teeth=20)
    b = pitch_radius(circ_pitch=5 * math.pi, teeth=20)
    c = pitch_radius(diam_pitch=25.4 / 5, teeth=20)
...
def test_gear_dist_no_shift_is_sum_of_pitch_radii() -> None:
    diameter = gear_dist(30, 15, mod=5, profile_shift1=0, profile_shift2=0)
    assert diameter == pytest.approx(pitch_radius(mod=5, teeth=30) + pitch_radius(mod=5, teeth=15))


def test_gear_dist_rack_uses_pitch_radius() -> None:
    # teeth2=0 is a rack; distance is the gear's pitch radius (+ shift)
    diameter = gear_dist(20, 0, mod=5, profile_shift1=0, profile_shift2=0)
    assert diameter == pytest.approx(pitch_radius(mod=5, teeth=20))
...
def test_internal_spur_gear_teeth_point_inward() -> None:
    ...
    pr = pitch_radius(mod=5, teeth=30)
    assert root_radius(mod=5, teeth=30, internal=True) < pr < outer_radius(mod=5, teeth=30, internal=True)
...
def test_gear_dist_with_profile_shift_increases_spacing() -> None:
    ...
    base = gear_dist(20, 20, mod=5, profile_shift1=0, profile_shift2=0)
    shifted = gear_dist(20, 20, mod=5, profile_shift1=0.5, profile_shift2=0.5)
    assert shifted > base
