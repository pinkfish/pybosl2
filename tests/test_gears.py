...
@pytest.mark.parametrize("kw", [
    {"pitch": 5, "teeth": 20, "face_width": 10, "pitch_angle": 45, "cutter_radius": 0},  # straight
    ...
    {"pitch": 5, "teeth": 20, "face_width": 10, "pitch_angle": 45, "left_handed": True},
])
...
def test_herringbone_envelope_matches_spur():
    h = G.herringbone_gear(mod=5, teeth=20, thickness=10)   # no helical -> matches the spur envelope
    w, l, hgt = _size(h)
...
def test_profile_shift_grows_the_tooth():
    ...
    r0 = max(math.hypot(x, y) for x, y in G.gear_tooth_profile(mod=5, teeth=8, profile_shift=0))
    r1 = max(math.hypot(x, y) for x, y in G.gear_tooth_profile(mod=5, teeth=8, profile_shift=0.5))
    assert r1 > r0
...
def test_gear_dist_no_shift_is_sum_of_pitch_radii():
    ...
    assert d == pytest.approx(G.pitch_radius(mod=5, teeth=30) + G.pitch_radius(mod=5, teeth=15))
...
def test_spur_gear_new_api_builds():
    assert isinstance(G.spur_gear(mod=5, teeth=18, thickness=25, helical=-29, shaft_diam=15), Bosl2Solid)
    assert isinstance(G.spur_gear(mod=5, teeth=16, thickness=35, helical=-20, herringbone=True), Bosl2Solid)
...
def test_profile_shift_gears_build(ps):
    ...
    assert isinstance(G.herringbone_gear(pitch=5, teeth=8, thickness=6, helical=20, profile_shift=ps), Bosl2Solid)
