...
def test_chamfer_replaces_each_corner_with_two_points():
    out = Path(SQ).round_corners(method="chamfer", joint=6)
...
def test_open_path_leaves_endpoints():
    out = Path([[0, 0], [40, 0], [40, 30], [0, 30]], closed=False).round_corners(radius=5)
...
def test_radius_requires_circle_method():
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(method="smooth", radius=5)


def test_width_requires_chamfer_method():
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(method="circle", width=5)


def test_k_requires_smooth_method():
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(method="circle", cut=3, curvature=0.5)
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(method="circle", cut=3, k=0.5)


def test_exactly_one_size_measure():
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(radius=5, cut=3)
    with pytest.raises(AssertionError):
        Path(SQ).round_corners()


def test_too_short_path_raises():
    with pytest.raises(AssertionError):
        Path([[0, 0], [10, 0]]).round_corners(radius=1)


def test_oversized_roundover_raises():
    # a radius bigger than the sides can't fit
    with pytest.raises(AssertionError):
        Path(SQ).round_corners(method="smooth", cut=10)
...
def test_smooth_path_closed_drops_duplicate_end():
    out = Path(SQ, closed=True).smooth_path(relsize=0.3, closed=True)
    assert out.closed is True
...
def test_path_join_plain_concatenation():
    p1 = [[0, 0], [10, 0]]
    p2 = [[10, 0], [20, 10]]
    res = Path(p1, closed=False).path_join([Path(p2, closed=False)], relocate=True)
...
def test_path_join_relocate_false():
    p1 = [[0, 0], [10, 0]]
    p2 = [[10, 0], [20, 10]]
    res = Path(p1, closed=False).path_join([Path(p2, closed=False)], relocate=False)
...
def test_path_join_with_rounding():
    ...
    p1 = [[0, 0], [10, 0]]
    p2 = [[10, 0], [10, 10]]
    res = Path(p1, closed=False).path_join([Path(p2, closed=False)], radius=2)
    assert len(res) > 3
