...
class TestCuboid:
    def test_sharp_box_matches_reference_formula(self):
        ...
        for p in [(4.9, 0, 0), (0, -4.9, 0), (0, 0, 4.9), (0, 0, 0), (2, 2, 2)]:
            assert math.isclose(float(shape.sample(*p)), float(_sharp_box_sdf(p, b)), abs_tol=10 ** (-9))
    ...
    def test_edges_all_rounding_matches_classic_formula(self):
        ...
        for p in [
            ...
            assert math.isclose(float(shape.sample(*p)), float(_round_box_sdf(p, b, r)), abs_tol=10 ** (-9))
