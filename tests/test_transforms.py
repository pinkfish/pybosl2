...
def test_rot_from_to_perpendicular() -> None:
    angle, axis = rot_from_to([1, 0, 0], [0, 1, 0])
    assert math.isclose(angle, 90.0)
    np.testing.assert_allclose(np.abs(axis), [0, 0, 1], atol=1e-9)


def test_rot_from_to_parallel_is_zero() -> None:
    angle, _ = rot_from_to([0, 0, 1], [0, 0, 5])
    assert math.isclose(angle, 0.0, abs_tol=1e-9)


def test_rot_from_to_antiparallel_is_180() -> None:
    angle, axis = rot_from_to([0, 0, 1], [0, 0, -1])
    assert math.isclose(angle, 180.0)
    assert math.isclose(float(np.linalg.norm(axis)), 1.0)


def test_axis_angle_matrix_is_rotation() -> None:
...
def test_rot_about_axis_through_point() -> None:
    m = rot_about_axis(90, [0, 0, 1], center=[5, 0, 0])  # type: ignore[arg-type]
    np.testing.assert_allclose(apply(m, [5, 0, 0]), [5, 0, 0], atol=1e-9)  # type: ignore
    np.testing.assert_allclose(apply(m, [6, 0, 0]), [5, 1, 0], atol=1e-9)  # type: ignore[arg-type, list-item]


def test_rot_inverse_undoes_transform() -> None:
    m = rot_about_axis(37, [0.3, 0.5, 0.8], center=[2, -1, 4])  # type: ignore[arg-type]
    np.testing.assert_allclose(rot_inverse(m) @ m, np.eye(4), atol=1e-9)


def test_rot_decode_round_trip() -> None:
    m = rot_about_axis(40, [0, 0, 1], center=[5, 0, 0])  # type: ignore[arg-type]
    angle, axis, center, axial = rot_decode(m)
    assert math.isclose(angle, 40.0, abs_tol=1e-6)
    np.testing.assert_allclose(axis, [0, 0, 1], atol=1e-9)
    np.testing.assert_allclose(center[:2], [5, 0], atol=1e-6)
    np.testing.assert_allclose(axial, [0, 0, 0], atol=1e-9)


def test_rot_decode_identity_is_zero_angle() -> None:
    angle, axis, center, axial = rot_decode(np.eye(4))
    assert math.isclose(angle, 0.0, abs_tol=1e-9)
