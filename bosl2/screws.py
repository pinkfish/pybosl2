...
class Screws:
    ...
    def screw(
        ...
        *length* is the shaft length below the head (for a flat head, below the surface). Set
        ...
        if thread:
            ...
            if shank_len > 1e-9:
                shank = cyl(diameter=d, height=shank_len, _fn=_fn, _fa=_fa, _fs=_fs).down(
                    shank_len / 2
                )
                shaft = shaft | shank
        else:
            shaft = cyl(diameter=d, height=length, _fn=_fn, _fa=_fa, _fs=_fs).down(length / 2)
    ...
    def _make_head(info, _fn, _fa, _fs):
        ...
        hs = info["head_size"]
        if head == "hex":
            return regular_prism(6, height=hh, inner_diameter=hs, _fn=_fn, _fa=_fa, _fs=_fs).up(hh / 2)
        if head in ("socket", "socket ribbed"):
            return cyl(diameter=hs, height=hh, chamfer2=hs / 20, _fn=_fn, _fa=_fa, _fs=_fs).up(
                hh / 2
            )
        if head == "button":
            rnd = min(hh * 0.9, hs / 2 * 0.9)
            return cyl(diameter=hs, height=hh, rounding2=rnd, _fn=_fn, _fa=_fa, _fs=_fs).up(hh / 2)
        if head in ("pan", "round"):
            return cyl(diameter=hs, height=hh, rounding2=0.2 * hs, _fn=_fn, _fa=_fa, _fs=_fs).up(
                hh / 2
            )
        if head == "flat":
            # 90-degree countersunk cone: shaft diameter at the bottom, head diameter at the surface.
            return cyl(diameter1=info["diameter"], diameter2=hs, height=hh, _fn=_fn, _fa=_fa, _fs=_fs).up(
                hh / 2
            )
        return None
    ...
    def _make_recess(info, head_top, _fn, _fa, _fs):
        ...
        eps = 0.02
        if drive == "hex":
            rec = regular_prism(6, height=depth + eps, inner_diameter=size, _fn=_fn, _fa=_fa, _fs=_fs)
        elif drive == "slot":
    ...
    def screw_hole(
        ...
        if thread:
            ...
            cutter = cyl(diameter=d + 2 * gap, height=length, _fn=_fn, _fa=_fa, _fs=_fs).down(
                length / 2
            )
        ...
        if head == "flat":
            ...
            hs = info["head_size"]
            csk_h = (hs - d) / 2
            csink = cyl(diameter1=d, diameter2=hs, height=csk_h + 0.02, _fn=_fn, _fa=_fa, _fs=_fs).up(
                (csk_h + 0.02) / 2 - 0.01
            )
            cutter = cutter | csink
            ...
            cb = cyl(diameter=hd, height=counterbore + 0.02, _fn=_fn, _fa=_fa, _fs=_fs).up(
