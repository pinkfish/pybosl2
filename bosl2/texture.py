...
def _tex_wave_ribs(n=None, **_):
    ...
    return [[(math.cos(math.radians(a)) + 1) / 2 for a in np.arange(0, 360 - 1e-9, 360 / n)]]


def _tex_diamonds(n=None, **_):
    n = _quantup(n if n is not None else 2, 2)
    path = _lerpn(0, 1, n // 2, False) + _lerpn(1, 0, n // 2, False)
    return [[min(_sel(path, i + j), _sel(path, i - j)) for j in range(n)] for i in range(n)]


def _tex_pyramids(n=None, **_):
    n = _quantup(n if n is not None else 2, 2)
    return [[1 - (max(abs(i - n / 2), abs(j - n / 2)) / (n / 2)) for j in range(n)] for i in range(n)]


def _tex_trunc_pyramids(n=None, **_):
    n = _quantup(n if n is not None else 6, 3)
    return [[(1 - (max(n / 6, abs(i - n / 2), abs(j - n / 2)) / (n / 2))) * 1.5
...
def _tex_diamonds_vnf(**_):
    verts = [[0, 1, 1], [0.5, 1, 0], [1, 1, 1],
             [0, 0.5, 0], [0.5, 0.5, 1], [1, 0.5, 0],
             [0, 0, 1], [0.5, 0, 0], [1, 0, 1]]
    faces = [[0, 1, 3], [2, 5, 1], [8, 7, 5], [6, 3, 7],
...
def _tex_cubes_vnf(**_):
    verts = [[0, 1, 0.5], [1, 1, 0.5], [0.5, 5 / 6, 1], [0, 4 / 6, 0], [1, 4 / 6, 0],
             [0.5, 3 / 6, 0.5], [0, 2 / 6, 1], [1, 2 / 6, 1], [0.5, 1 / 6, 0], [0, 0, 0.5], [1, 0, 0.5]]
    faces = [[0, 1, 2], [0, 2, 3], [1, 4, 2], [2, 5, 3], [2, 4, 5], [6, 3, 5],
             [4, 7, 5], [7, 8, 5], [6, 5, 8], [10, 8, 7], [9, 6, 8], [10, 9, 8]]
    return verts, faces
...
def _tex_bricks_vnf(border=None, gap=None, **_):
    ...
    verts = (_sqr(1)
             ...
             + _mv([0.5 + g / 2 + b / 2, 0.5 + g / 2 + b / 2, 1], _sqr([0.5 - g / 2 - b / 2, 0.5 - g - b])))
    faces = [[0, 4, 7, 20], [4, 8, 11, 7], [9, 8, 4, 5], [4, 0, 1, 5], [10, 9, 5, 6],
             [20, 7, 6, 13, 12, 21], [2, 3, 23, 22, 15, 14], [15, 19, 18, 14], [22, 23, 27, 26],
             [16, 19, 15, 12], [13, 6, 5, 1], [26, 25, 21, 22], [8, 9, 10, 11], [7, 11, 10, 6],
             [17, 16, 12, 13], [22, 21, 12, 15], [16, 17, 18, 19], [24, 25, 26, 27], [25, 24, 20, 21]]
    return verts, faces
...
def _tex_checkers_vnf(border=None, **_):
    ...
    verts = (_mv([0, 0], _sqr(0.5 - b, 1)) + _mv([0, 0.5], _sqr(0.5 - b))
             + _mv([0.5, 0], _sqr(0.5 - b)) + _mv([0.5, 0.5], _sqr(0.5 - b, 1))
             + [[0.5 - b / 2, 0.5 - b / 2, 0.5], [0, 1, 1], [0.5 - b, 1, 1],
                [0.5, 1, 0], [1 - b, 1, 0], [1, 0, 1], [1, 0.5 - b, 1],
                [1, 0.5, 0], [1, 1 - b, 0], [1, 1, 1], [0.5 - b / 2, 1 - b / 2, 0.5],
                [1 - b / 2, 1 - b / 2, 0.5], [1 - b / 2, 0.5 - b / 2, 0.5]])
...
def _tex_trunc_diamonds_vnf(border=None, **_):
    ...
    for i in range(4):
        j = i * 3 + 8
        faces += [[i, (i + 1) % 4, (i + 1) % 4 + 4, i + 4], [j, j + 1, j + 2], [i, (i + 3) % 4, j + 1, j]]
    faces.append([4, 5, 6, 7])
    return verts, faces
...
def _tex_tri_grid_vnf(border=None, **_):
    ...
    verts = [[0, 0, 0], [1, 0, 0], [adj, y1, 1], [1 - adj, y1, 1], [0, y2, 1], [1, y2, 1],
             [0.5, 0.5 - y2, 1], [0, y3, 1], [0.5 - adj, y3, 1], [0.5 + adj, y3, 1], [1, y3, 1],
             [0, 0.5, 0], [0.5, 0.5, 0], [1, 0.5, 0],
             [0, y4, 1], [0.5 - adj, y4, 1], [0.5 + adj, y4, 1], [1, y4, 1],
             [0.5, 0.5 + y2, 1], [0, y5, 1], [1, y5, 1], [adj, y6, 1], [1 - adj, y6, 1], [0, 1, 0], [1, 1, 0]]
    faces = [[0, 2, 3, 1], [21, 23, 24, 22], [2, 6, 3], [0, 12, 6, 2], [1, 3, 6, 12],
             [0, 4, 8, 12], [4, 7, 8], [8, 7, 11, 12], [1, 12, 9, 5], [5, 9, 10], [10, 9, 12, 13],
             [11, 14, 15, 12], [19, 15, 14], [19, 23, 12, 15], [16, 17, 13, 12], [16, 20, 17],
             [12, 24, 20, 16], [21, 22, 18], [12, 23, 21, 18], [12, 18, 22, 24]]
    return verts, faces
...
def _circle_xy(d, n):
    ...
    return [[d / 2 * math.cos(math.radians(360 * i / n)), d / 2 * math.sin(math.radians(360 * i / n))]
            for i in range(n)]
...
def _sph(r, theta, phi):
    ...
    return [r * math.cos(t) * math.sin(p), r * math.sin(t) * math.sin(p), r * math.cos(p)]
...
def _base_faces(n, base0, border):
    ...
    for i in range(4):
        ...
        if border > 0:
            out.append(arc + [(2 * i + 7) % 8 + base0, (2 * i) % 8 + base0, (2 * i + 1) % 8 + base0])
        else:
...
def _tex_hex_grid_vnf(border=None, **_):
    ...
    sc = 1 / 3 / hyp
    hex_ = [[1, 2 / 6, 0], [0.5, 1 / 6, 0], [0, 2 / 6, 0], [0, 4 / 6, 0], [0.5, 5 / 6, 0], [1, 4 / 6, 0]]
    R = (0.5 - b) / math.cos(math.radians(30))               # ellipse(circum, $fn=6) vertex radius
    top = [[0.5 + R * math.cos(math.radians(-30 + 60 * i)),
            0.5 + R * math.sin(math.radians(-30 + 60 * i)) * sc, 1.0] for i in range(6)]

    def cyl(rad, ang):                                       # yscale(sc, cylindrical_to_xyz(rad, ang, 1))
        return [rad * math.cos(math.radians(ang)), rad * math.sin(math.radians(ang)) * sc, 1.0]

    def add(a, b3):
        return [a[0] + b3[0], a[1] + b3[1], a[2] + b3[2]]

    verts = [list(h) for h in hex_] + top + [
        add(hex_[0], [0, -diag * sc, 1]),
        add(hex_[1], cyl(diag, 270 + 60)), add(hex_[1], cyl(diag, 270 - 60)),
        add(hex_[2], [0, -diag * sc, 1]),
        [0, 0, 1], [0.5 - b, 0, 1], [0.5, 0, 0], [0.5 + b, 0, 1], [1, 0, 1],
        add(hex_[3], [0, diag * sc, 1]),
        add(hex_[4], cyl(diag, 90 + 60)), add(hex_[4], cyl(diag, 90 - 60)),
        add(hex_[5], [0, diag * sc, 1]),
        [0, 1, 1], [0.5 - b, 1, 1], [0.5, 1, 0], [0.5 + b, 1, 1], [1, 1, 1],
    ]
    faces = [[6, 7, 8, 9, 10, 11]]
    faces += [[i, (i + 1) % 6, (i + 1) % 6 + 6, i + 6] for i in range(6)]
    faces += [[20, 19, 13, 12], [17, 16, 15, 14], [21, 25, 26, 22], [23, 28, 29, 24],
              [0, 12, 13, 1], [1, 14, 15, 2], [3, 21, 22, 4], [4, 23, 24, 5],
              [1, 13, 19, 18], [1, 18, 17, 14], [4, 22, 26, 27], [4, 27, 28, 23]]
    return verts, faces
...
def texture(tex, n=None, border=None, gap=None, roughness=None, inset=None, fn=None):
    ...
    *n* sets the resolution of the parametric height-field textures; *border*/*gap* shape the VNF-tile
    ...
    if key not in TEXTURES:
        raise ValueError(f"Unrecognized (or unported) texture name: {tex!r}; "
