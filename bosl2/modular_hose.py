...
_SMALL_CMDS = [
    (["left", 90 - 38.5, "arcsteps", 12, "arcleft", 6.38493, 62.15, "arcsteps", 4,
      "arcleft", .5, 90 + 38.5 - 62.15, "move", .76, "left", 67.5, "move", .47,
      "left", 90 - 67.5, "move", 4.165, "right", 30, "move", 2.1], 4.864),
    (["left", 90 - 41, "arcsteps", 16, "arcleft", 10.7407, 64.27, "arcsteps", 4,
      "arcleft", .5, 90 + 41 - 64.27, "move", .95 - .4, "left", 45, "move", .4 * _SQRT2,
      "left", 45, "move", 7.643 - .4, "right", 30, "move", 4.06], 8.1),
    (["left", 90 - 30.4, "arcsteps", 16, "arcleft", 13.99219, 53, "arcsteps", 4,
      "arcleft", .47, 90 - 53 + 30.4, "move", .597, "left",
      "move", 9.908 - 1.905 / _tan(25) + 3.81 * _cos(30), "right", 25,
      "move", 1.905 / _sin(25)], 11.989),
]

# Socket ("big") end cross-section, one per size.
_BIG_CMDS = [
    (["left", 90 - 22, "move", 6.5, "left", .75, "arcsteps", 8, "arcleft", 6.5, 37.3,
      "setdir", 90, "move", .21, "right", "move", 1.24, "right", 45, "move", .7835,
      "right", 19, "move", 1.05, "setdir", -90, "move", 1, "right", 22, "move", 8.76], 3.268),
    (["left", "right", 22, "move", 9, "arcsteps", 8, "arcleft", 11, 36.5, "setdir", 90,
      "move", 2 - 1.366, "right", "move", .91, "arcsteps", 4, "arcright", 1.25, 90,
      "move", 2.2, "arcsteps", 8, "arcright", 13, 22.4, "move", 8.73], 6.42154),
    (["left", 90 - 22, "move", 7.633, "arcsteps", 16, "arcleft", 13.77, 35.27, "setdir", 90,
      "move", 1.09, "right", "move", 1.0177, "right", 45, "move", 1.009, "right", 77.8 - 45,
      "move", .3, "arcright", 15.5, 34.2, "move", 6.47], 9.90237),
]
...
class ModularHose:
    ...
    @staticmethod
    def modular_hose(size: float, type: str = "segment", clearance: float | list = 0,
                     waist_len: float | None = None, _fn: int | None = None) -> Bosl2Solid:
        ...
        *size* is 0.25, 0.5 or 0.75 (the 1/4", 1/2", 3/4" hose families). *type* is ``"ball"``/
        ...
        if type == "segment":
            ...
            shape = [[x, y + mid] for x, y in smallend] + [[smallend[-1][0], 0], [smallend[0][0], 0]]
        elif type in ("big", "socket"):
            shape = [[x, y + mid] for x, y in bigend] + [[bigend[-1][0], 0], [bigend[0][0], 0]]
        else:
        ...
        solid = _orotate_extrude(_opolygon(poly), fn=_fn) if _fn else _orotate_extrude(_opolygon(poly))
