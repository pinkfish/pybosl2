...
_LMXUU = {
    4: LinearBearingSpec(8, 12), 5: LinearBearingSpec(10, 15), 6: LinearBearingSpec(12, 19),
    8: LinearBearingSpec(15, 24), 10: LinearBearingSpec(19, 29), 12: LinearBearingSpec(21, 30),
    13: LinearBearingSpec(23, 32), 16: LinearBearingSpec(28, 37), 20: LinearBearingSpec(32, 42),
    25: LinearBearingSpec(40, 59), 30: LinearBearingSpec(45, 64), 35: LinearBearingSpec(52, 70),
    40: LinearBearingSpec(60, 80), 50: LinearBearingSpec(80, 100), 60: LinearBearingSpec(90, 110),
    80: LinearBearingSpec(120, 140), 100: LinearBearingSpec(150, 175),
}
...
class LinearBearings:
    ...
    @staticmethod
    def linear_bearing(l: float = 24, od: float = 15, id: float = 8,
                       color: str | None = "silver") -> Bosl2Solid:
        """A generic linear ball-bearing cartridge, bore *id* / outer *od* / length *l* (BOSL2 linear_bearing()).
        ...
        """
        body = _union([
            tube(id=id, od=od, h=l - 1),
            tube(id=od - 1, od=od, h=l),
            tube(id=id, od=id + 1, h=l),
            tube(id=id + 2, od=od - 2, h=l),
        ])
        result = Bosl2Solid(body.shape, size=[od, od, l])
    ...
    def lmXuu_bearing(size: int = 8, color: str | None = "silver") -> Bosl2Solid:
        ...
        return LinearBearings.linear_bearing(l=spec.length, id=size, od=spec.od, color=color)

    @staticmethod
    def linear_bearing_housing(d: float = 15, l: float = 24, tab: float = 8, gap: float = 5,
                               wall: float = 3, tabwall: float = 5, screwsize: float = 3,
                               _fn: int | None = None) -> Bosl2Solid:
        """A pillow-block housing that clamps a linear bearing (bore *d*, length *l*) to a plate (BOSL2 linear_bearing_housing()).
        ...
        body = _union([
            teardrop(d=od, h=l).rotate([0, 90, 0]),                # teardrop shell, axis along X
            cuboid([l, od, od / 2]).down(od / 4),                  # base
            cuboid([l, ogap, od / 2 + tab / 2]).up((od / 2 + tab / 2) / 2),  # clamp tabs
        ])
        body = body - teardrop(d=d, h=l + 0.1).rotate([0, 90, 0])   # bearing bore
        body = body - cuboid([l + 0.1, gap, od])                            # split gap
        # clamp screw across the tabs (a simple clearance hole)
        from bosl2.screws import Screws
        screw = Screws.screw_hole(f"M{screwsize:g}", length=ogap + 1, _fn=_fn or 16).rotate([90, 0, 0]).up(tabh)
        body = body - screw
        return Bosl2Solid(body.shape, size=[l, od, od + tab / 2])

    @staticmethod
    def lmXuu_housing(size: int = 8, tab: float = 7, gap: float = 5, wall: float = 3,
                      tabwall: float = 5, screwsize: float = 3, _fn: int | None = None) -> Bosl2Solid:
        """A pillow-block housing sized for a standard LMxUU bearing (BOSL2 lmXuu_housing())."""
        spec = LinearBearings.lmXuu_info(size)
        return LinearBearings.linear_bearing_housing(d=spec.od, l=spec.length, tab=tab, gap=gap,
                                                     wall=wall, tabwall=tabwall, screwsize=screwsize, _fn=_fn)
