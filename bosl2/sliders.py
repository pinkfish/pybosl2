...
class Sliders:
    ...
    @staticmethod
    def slider(l: float = 30, w: float = 10, h: float = 10, base: float = 10, wall: float = 5,
               ang: float = 30, slop: float = 0.0) -> Bosl2Solid:
        ...
        full_width = w + 2 * wall
        full_height = h + base
        parts = [cuboid([full_width, l, base - slop], chamfer=2, edges=[FRONT, BACK],
                        except_edges=[BOTTOM], anchor=BOTTOM)]
        for m in xflip_copy(offset=w / 2 + slop):
            wallcube = cuboid([wall, l, full_height], chamfer=2, edges=[RIGHT],
                              except_edges=[BOTTOM], anchor=[b + le for b, le in zip(BOTTOM, LEFT)])
            parts.append(wallcube.multmatrix(m.tolist()))
        bev_h = h / 2 * math.tan(math.radians(ang))
        for m in xflip_copy(offset=w / 2 + slop + 0.02):
            slid = prismoid([h, l], [0, l - w], h=bev_h + 0.01, orient=LEFT, anchor=BOTTOM)
    ...
    @staticmethod
    def rail(l: float = 30, w: float = 10, h: float = 10, chamfer: float = 1.0,
             ang: float = 30) -> Bosl2Solid:
        ...
        pts = [
            [-x5, -y1, z3], [x5, -y1, z3], [x7, -y1, z4], [x4, -y1, -z1 - 0.05],
            [-x4, -y1, -z1 - 0.05], [-x7, -y1, z4],
            [-x3, -y2, z1], [x3, -y2, z1], [x2, -y2, z2], [x6, -y2, z4],
            [x1, -y2, -z1 - 0.05], [-x1, -y2, -z1 - 0.05], [-x6, -y2, z4], [-x2, -y2, z2],
            [x5, y1, z3], [-x5, y1, z3], [-x7, y1, z4], [-x4, y1, -z1 - 0.05],
            [x4, y1, -z1 - 0.05], [x7, y1, z4],
            [x3, y2, z1], [-x3, y2, z1], [-x2, y2, z2], [-x6, y2, z4],
            [-x1, y2, -z1 - 0.05], [x1, y2, -z1 - 0.05], [x6, y2, z4], [x2, y2, z2],
        ...
        faces = [
            ...
            [0, 13, 6], [0, 6, 7], [0, 7, 1], [1, 7, 8], [1, 8, 9], [1, 9, 2],
            [2, 9, 10], [2, 10, 3], [3, 10, 11], [3, 11, 4], [4, 11, 12], [4, 12, 5],
            [5, 12, 13], [5, 13, 0],
            [14, 15, 16], [14, 16, 19], [16, 17, 18], [16, 18, 19],
            [14, 27, 20], [14, 20, 21], [14, 21, 15], [15, 21, 22], [15, 22, 23], [15, 23, 16],
            [16, 23, 24], [16, 24, 17], [17, 24, 25], [17, 25, 18], [18, 25, 26], [18, 26, 19],
            [19, 26, 27], [19, 27, 14],
            [6, 21, 20], [6, 20, 7], [7, 20, 27], [7, 27, 8], [8, 27, 26], [8, 26, 9],
            [9, 26, 25], [9, 25, 10], [10, 25, 24], [10, 24, 11], [11, 24, 23], [11, 23, 12],
            [12, 23, 22], [12, 22, 13], [13, 22, 21], [13, 21, 6],
        ]
