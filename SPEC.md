...
| 3 | **C-8** | **Three public functions take a `(verts, faces)` pair instead of a `VNF`** — `texture.is_watertight_topology`, `rasterize_vnf_texture` and `vnf_tile_to_solid`. C-8 makes `VNF` the vertex/face mesh interchange type, and these predate it: they thread the two halves of a mesh through separately, so a caller can pass vertices from one mesh and faces from another and get silent nonsense. Found while closing T24 — a mesh vertex array is *not* a polyline (reordering `verts` without reindexing `faces` destroys the geometry), so C-7a is the wrong requirement for them and C-8 is the right one. They are listed as C-7a carve-outs in `tests/test_polyline_parameters.py` with a pointer to this row. |
