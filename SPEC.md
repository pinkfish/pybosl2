...
|---|---|---|
| 1 | **PAR-5** | The SDF `pie_slice` stores the full disc's bounding box rather than the wedge's, so `bounds()` over-reports on a shape whose selling point is exact bounds. `tests/test_backend_parity.py::BOUNDS_NOT_YET_EXACT` pins it. |
| 2 | **S-46a / PAR-1** | Parts refuse on the SDF backend rather than building: none of the 53 has an SDF form. Closing this means expressing the ones that can be (simple prisms, bearings, hoses) through the façade, and keeping the refusal only where a part genuinely needs CSG-only operations. |
