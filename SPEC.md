...
| 5 | **Q-7** (new) | **The distribution is untested, and the typing does not reach users.** The package ships no `py.typed`, so PEP 561 makes every type checker ignore it: `mypy --strict`, the five `.pyi` stubs, C-7a's "the point type is the contract" and DOC-5's type-checked examples are all invisible to anyone who `pip install`s this. Q-1…Q-6 test the source tree and nothing tests the artifact — the committed `dist/` wheel still carries `solid.pyi`, deleted from the source under PLAN T-8. The gate: build the wheel, install it into a clean virtualenv, import it, and run `mypy --strict` over a consumer snippet. T28. |
