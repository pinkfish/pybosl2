...
|---|---|---|
| 1 | **C-14 / O-2** | **16 of the 51 parts build their geometry in `__init__`** (was 45), where C-14 says a part "builds geometry lazily under a `shape` property... callers can therefore *measure* a part without building it". Nothing measured the claim, so it sat in the spec being false. The cost is real but smaller than the count suggests, and worth stating precisely: most eager parts expose nothing worth querying — `SpurGear` has `teeth` and `shape` — so there is no catalogue lookup to make cheap. It bites on the **17** parts that expose three or more derived dimensions *and* build eagerly: asking `Slider` for its `length` costs a full 18 ms build, and querying all 17 costs ~31 ms against roughly nothing. `Screw`, already lazy, answers `.pitch` in a microsecond and defers its 11 ms build. Measured and held by `tests/test_parts_are_lazy.py`, whose list only shrinks — 16 left, from 45. `Slider` was converted by hand and 25 more by a transformation that grew a guard for each way the split can go wrong. **Converting one is not the mechanical move it looks like:** an automated attempt across six files split `__init__` at the resolved-spec assignments and carried the *validation* into the deferred build with the geometry, so a rejected call stopped raising until someone asked for a shape (45 tests caught it, and it was reverted). A part separates **three** things, not two — validate the arguments, resolve the derived dimensions, then build. Two further reasons the rest resist a script: a part may resolve a derived attribute *in the middle of* its geometry, its validation may be indirect — a spec-object constructor that raises, which no scan for a literal `raise` finds; and a trailing `self._x = y` assignment is only spec if `y` is a constructor *parameter*, not a local the geometry computed. Making the parts lazy also surfaced **three** refusal messages naming the wrong reason: `Worm`, `BevelGear` and `WormGear` all claimed to build "2-D geometry" when all three build a VNF — one copy-paste propagated three times, invisible while the refusal fired generically at construction. |
