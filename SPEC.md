...
* **C-19 Colour and distribution belong to the shared contract.** Both are operations on "any
  ...
  universal now: the SDF backend's solids carry colour as field metadata, and its 2-D shapes gained
  the same shared mixins the solids had (`Colorable`, `Anchorable`, `Distributable`). The gap was
  never a limit of distance fields — colour is metadata, distribution is matrices, and anchoring
  needs only a bounding box, which an SDF carries exactly. It was the 2-D shape having inherited
  none of it.
...
* **C-7a** A public parameter that means *an ordered set of points* — a path, outline, profile,
  polygon or point list — MUST be typed as `Path2D`, `Path3D` or `Path`, and MUST NOT accept a
  bare `Sequence[Sequence[float]]` or a NumPy array. **The point type is the contract, not a
  convenience wrapper over one.** A bare sequence carries no dimension, no open/closed flag and no
  winding, so each function that takes one has to re-derive all three, and they do not always
  agree — the same list is a 2-D outline to one function and a degenerate 3-D path to the next.
  Requiring the type moves that decision to the one place that can make it once, at construction.
  A caller with raw points writes `Path2D(pts)`, which is where the points get checked.
* **C-7d** Where only one width will do, the check MUST be for the *concrete* type, not for
  `Path`. `Path2D` and `Path3D` are siblings, so a guard that asks only "is this a `Path`?"
  lets a `Path3D` satisfy a `Path2D` parameter and reach a formula that reads columns 0 and 1
  and drops z — a wrong answer rather than a refusal, which is worse than the bare sequence
  this requirement set out to remove, because it is silent. `require_path(..., expect=Path2D)`
  performs it, and says how to convert.
* **C-7b** The refusal MUST name the wrapper to apply. Raw points are what a newcomer has —
  literals, a CSV, another library's output — so `polygon([[0,0],[1,0],[1,1]])` is the natural
  first attempt, and the error it produces is the library's chance to teach the type rather than
  merely reject the call (E-4).
* **C-7c** `PathLike` remains, but is **internal**: it types the normalisation step inside a
  function body, never a public parameter. Its docstring said it was "anything an API that wants a
  polyline accepts", which made the permissive form look like the intended contract at every call
  site that imported it.
...
* **S-23a One family, one kind of answer.** A stroke is the **area the pen covers**, so in 2-D it
  is a `Region` — outlines-with-holes, which is what a dashed stroke needs and what a stroked
  region already returned — and in 3-D a `Solid`. `Path2D.stroke()` returning a `Path2D` described
  the *boundary* rather than the mark, and left one member of a four-member family answering a
  different kind of thing from the other three (`Path2D.dashed_stroke`, `Region.stroke`,
  `Region.dashed_stroke` all return `Region`; `Path3D.stroke` returns `Solid`). The underlying
  outline stays reachable through the region's `paths`.
...
* **DOC-1a Generation MUST NOT destroy what it did not write.** The generator owns the module
  stubs and `index.rst`; everything else is prose it is a guest among. A page it does not
  recognise is *deleted*, and a hand-edit to a generated file is *overwritten* — both silently,
  because neither raises a warning and a build that removes a page still reports success. So a
  hand-written page is registered with the generator, a link to it belongs in the generator rather
  than in its output, and a test — not a convention — is what keeps the register honest.
...
* **Q-7** The **artifact** passes, not only the source tree. A gate builds the wheel, installs it
  into a clean virtualenv, imports it, and type-checks a short consumer snippet against it. Q-1…Q-6
  all read the working copy, so nothing before this asked what a release actually contains — which
  is how `py.typed` came to be missing from every wheel this project has published while
  `mypy --strict` ran clean on every commit. What ships is a separate claim from what builds, and
  an unmeasured claim is a wish (B2-1).
...
| 3 | **SPEC/PLAN maintainability** | The documents that govern this project are the one part of it nothing measures. 263 numbered requirements across the two, of which **12 have been triaged** and 251 have not: nobody has decided whether a test guards them, a reviewer does, or nothing does. Five prefixes collide across the two documents — `S-2` is "every shape reports its bounds" in this file and "functions stay under 50 lines" in [PLAN.md](PLAN.md), and `T`, `L`, `O` and `Q` are the same story — so a citation in a commit body, which §13 rule 1 asks for, can be permanently ambiguous. Eight rules are stated twice, once here and once as their Python mechanics (C-7a↔T-4, S-19b↔T-6d, E-1↔E-P4a, C-14↔O-2, S-46a↔O-0a, A-6↔M-2a, R-1↔R-P2, D-5↔E-P5), so each change is two edits and two chances to disagree. And §12.1 below is 40 % of this file by volume: valuable history, sitting in the path of every reader looking for the contract. The registry (`spec/requirements.toml`, T26) holds each requirement as data — statement, keyword, section, the test that enforces it, its triage status — and `tests/test_requirements.py` checks that ids are unique, that every `enforced_by` target exists, that all 547 document-qualified citations in the package and the tests resolve, and that the untriaged backlog only shrinks. T27 generates the prose from it and moves §12.1 to `CONFORMANCE.md` and the rationale to `docs/rationale/`. |
