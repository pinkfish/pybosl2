...
| 3 | **C-7a** | **10 public parameters that mean a sequence of points still accept a bare sequence or a NumPy array** (was 51; T24), measured by `tests/test_polyline_parameters.py`, whose list only shrinks. C-7 asked only that such an API *accept* a `Path`; every one of these does, via the permissive `PathLike` alias, and that is exactly the problem — the widest form is also the default one, so `PathLike` spread to public signatures as though it were the contract. Its own docstring described it as "anything an API that wants a polyline accepts", which is how it got there. A bare sequence drops the three things the callee needs — dimension, closed-ness, winding — so each function re-derives them and they disagree: the same list is a 2-D outline to one and a degenerate 3-D path to the next. **The count is the work; the reason is that the permissive alias was never marked as internal.** The starting number was itself measured wrong at first (36): the scan missed `ArrayLike` entirely, though this requirement names a NumPy array explicitly, and it counted one function nested inside another as public API. Correcting the scan moved the denominator from 36 to 51 — the debt had always been that size. What the tranches buy is visible at each step: retyping `arc(points=)` made `mypy --strict` name three internal callers still passing raw lists and made `arc()`'s own "2-D points only" branch unreachable; typing the SDF entry points deleted `as_path_list()`'s guess at whether it held one outline or several, which is unanswerable for a 2-point argument, and retired `polygon_prism()`'s hand-rolled `TypeError` in favour of a message that names the wrapper (E-4). Converting the path utilities then exposed **C-7d**: `path_normals` is typed `Path2D` because rotating a tangent a quarter turn is a planar construction, and a `Path3D` was satisfying it silently — the guard checked for `Path`, and the two are siblings. |
