...
class _AabbSolid:
    ...
    def rotate(self, a, v=None):
        ...
        if mn is None or mx is None:
            return _AabbSolid()
        is_z_rot = False
        if v is not None and list(v) == [0, 0, 1]:
            is_z_rot = True
        elif isinstance(a, (list, tuple)) and len(a) == 3 and a[0] == 0 and a[1] == 0:
            is_z_rot = True
...
def install():
    ...
    setattr(pythonscad_mock, "rotate_extrude", _mock_rotate_extrude)
    setattr(pythonscad_mock, "polygon", _mock_polygon)
    for name in [
        "textmetrics",
        "square",
        "circle",
    ...
    setattr(openscad_mock, "polygon", _mock_polygon)
