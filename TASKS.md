...
**Risk:** medium — SPEC.md and PLAN.md become generated files, and a hand-edit to one is lost

1. `docs/_reqgen.py`, alongside `_covgen.py`/`_rstgen.py`/`_specgen.py` (PLAN D-P7), renders
   SPEC.md and PLAN.md from the registry, each with the "generated by" line D-P7 requires.
2. Trim each `statement` to the normative sentence or two, and move the rationale it carries to
   `docs/rationale/<ID>.md`, linked from the generated bullet. The rationale is the best writing
   in the repository; the problem is only that it sits in the path of a reader looking for the
   contract.
3. §12.1 moves verbatim to `CONFORMANCE.md`, append-only — 40 % of SPEC.md by volume, and history
   rather than contract. §12.2 stays in the spec: open debt is contract.
4. T0–T25 move to `docs/tasks-archive.md`, with their postmortems. This file holds open work.
5. `test_the_registry_and_the_prose_agree` strengthens into the generated-file equality check
   `_covgen.py` already uses, which is what DOC-1a asks for.
6. Regenerate `AGENTS.md`'s "short version" from the requirements marked as its core seven, so the
   one-page index cannot drift from what it indexes.

**Done when:** SPEC.md and PLAN.md are generated and the committed copies match; a hand-edit to
either fails the build; `CONFORMANCE.md` and `docs/tasks-archive.md` carry the history intact.
