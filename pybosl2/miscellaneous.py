...
class Extrudable:
    """Mixin adding path_extrude / path_extrude2d as methods on :class:`~pybosl2.path2d.Path2D` and.

    :class:`~pybosl2.path3d.Path3D`. Both take the 2-D cross-section as a *profile* argument instead
    ...
    **Both are CSG-only, and say so now.** They are built out of native primitives -- `path_extrude`
    ...
    def path_extrude2d(
        ...
        if s is None:
            ...
            s = math.hypot(max_x - min_x, max_y - min_y)
        ...
        for i in range(last):
            a = pts[i]
            b = pts[(i + 1) % sides]
            segv = [float(b[0]) - float(a[0]), float(b[1]) - float(a[1])]
            seglen = math.hypot(segv[0], segv[1])
            if seglen < 1e-9:
                continue
    ...
    def path_extrude(
        ...
        :func:`~pybosl2.skin.Sweepable.path_sweep` is faster and cleaner; this exists for extruding an arbitrary
        ...
        from pybosl2.shapes3d import Bosl2Solid
        from pybosl2.skin import rot_resample
        ...
        for i in range(sides - 1):
            vec1 = np.asarray(UP.vector, dtype=float) if i == 0 else unit(list(parr[i] - parr[i - 1]))
            vec2 = unit(list(parr[i + 1] - parr[i]))
