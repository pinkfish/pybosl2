...
class Shape(Protocol):
    ...
    # `Shape` spans both dimensions (C-22) and parameters are contravariant, so no single type
    # satisfies both -- `Path2D | Path3D` is *wider* than either implementation accepts, and each
    # class stops conforming. Measured in T76, which is why this one member keeps the loose form
    # while the five beside it were spelled out.
...
class Solid(Shape, Protocol):
    ...
    def offset3d(self, radius: float) -> Self: ...
    # The three radii are the shared contract. The CSG spelling also takes `size` and the facet
    # controls; a field has no facets to control (B-9), so those are not part of what both honour.
    def round3d(
        self,
        radius: float | None = None,
        *,
