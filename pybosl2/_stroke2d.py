...
def stroke(
    ...
    **kwargs: Any,
    ...
    """Convenience: stroke a raw path (or Path2D/Path3D/Region).

    Delegates to :meth:`Path2D.stroke` or :meth:`Path3D.stroke`.
    """
    from pybosl2.path2d import Path2D
    ...
    if isinstance(path, Region):
        return path.stroke(width=width, **kwargs)
    if isinstance(path, Path3D):
        return path.stroke(width=width, closed=closed, **kwargs)
    if isinstance(path, Path2D):
        return path.stroke(width=width, closed=closed, **kwargs)
    pts = list(path)
    if pts and len(pts[0]) >= 3:
        return Path3D(pts, closed=closed or False).stroke(width=width, **kwargs)
    return Path2D(pts, closed=closed or False).stroke(width=width, closed=closed, **kwargs)
...
def dashed_stroke(
    ...
    """Convenience: dash a raw path (or Path2D/Path3D/Region).

    Delegates to :meth:`Path2D.dashed_stroke` or :meth:`Path3D.dashed_stroke`.
    """
    from pybosl2.path2d import Path2D
    from pybosl2.path3d import Path3D
    from pybosl2.regions import Region
    ...
    if isinstance(path, Region):
        return path.dashed_stroke(dashpat=dashpat, closed=True, fit=fit, mindash=mindash)
    if isinstance(path, Path3D):
        return path.dashed_stroke(dashpat=dashpat, closed=closed, fit=fit, mindash=mindash)
    if isinstance(path, Path2D):
        return path.dashed_stroke(dashpat=dashpat, closed=closed, fit=fit, mindash=mindash)
    pts = list(path)
    is_3d = len(pts[0]) >= 3 if pts else False
    p = Path3D(pts, closed=closed or False) if is_3d else Path2D(pts, closed=closed or False)
    return p.dashed_stroke(dashpat=dashpat, closed=closed, fit=fit, mindash=mindash)
