...
:meth:`~pybosl2.color.Colorable.recolor` and :meth:`~pybosl2.color.Colorable.color_this` both apply the
...
.. list-table::
   ...
   * - ``highlight`` / ``highlight_this``
     - ported
     - :meth:`~pybosl2.color.Colorable.highlight` -- the ``#`` modifier (native ``highlight()``); the
       single-level ``highlight_this`` collapses to the same call.
   * - ``ghost`` / ``ghost_this``
     - ported
     - :meth:`~pybosl2.color.Colorable.ghost` -- the ``%`` modifier (native ``background()``).
   * - ``color_overlaps``
     - not ported
...
API reference
-------------
.. automodule:: pybosl2.color
   :members:
   :undoc-members:
   :exclude-members: Colorable

.. autoclass:: pybosl2.color.Colorable
   :members:
