...
.. list-table::
   ...
   * - **CSG only**
     - ``projection``, ``fill``, and all 2-D geometry
       (:class:`~bosl2.shapes2d.Bosl2Shape2D`, :meth:`Path.polygon() <bosl2.paths.Path.polygon>`,
       :meth:`Path.hull() <bosl2.paths.Path.hull>`, ``rotate_extrude``, a 2-D ``stroke()``)
...
2-D *geometry* is a CSG-only notion: :class:`~bosl2.shapes2d.Bosl2Shape2D` and every
...
.. list-table::
   ...
   * - Call
     - CSG
     - SDF
   * - :meth:`Path.linear_extrude() <bosl2.paths.Path.linear_extrude>`,
       :meth:`Region.linear_extrude() <bosl2.regions.Region.linear_extrude>`
     - native ``linear_extrude`` → :class:`~bosl2.shapes3d.Bosl2Solid`; takes
   ...
   * - :meth:`Bosl2Solid.hull() <bosl2.shapes3d.Bosl2Solid.hull>` /
       :meth:`PyShape.hull() <bosl2._sdf.shapes3d.PyShape.hull>`
     - exact native ``hull()``
     - polyhedral hull of the children's support points
   * - :func:`stroke() <bosl2.drawing.stroke>` of a **3-D** path
     - a tube of Bosl2Solid cylinders/spheres
     - the same tube as one distance field
   * - :meth:`Bosl2Solid.projection() <bosl2.shapes3d.Bosl2Solid.projection>`,
       ``Path.polygon()``/``.fill()``/``.hull()``/``.rotate_extrude()``, 2-D ``stroke()``
     - → :class:`~bosl2.shapes2d.Bosl2Shape2D` / :class:`~bosl2.shapes3d.Bosl2Solid`
     - :class:`~bosl2.exceptions.UnsupportedByBackend`
